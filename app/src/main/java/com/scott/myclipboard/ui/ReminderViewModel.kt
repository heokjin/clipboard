...
import com.scott.myclipboard.reminder.ReminderStyleSpan
import com.scott.myclipboard.reminder.adjustReminderSpansForTextChange
import com.scott.myclipboard.reminder.encodeReminderStyleSpans
import com.scott.myclipboard.reminder.styleSpans
import com.scott.myclipboard.reminder.summarizeReminderSelectionStyle
import com.scott.myclipboard.reminder.toggleReminderHighlightOnSelection
import com.scott.myclipboard.reminder.trimReminderTextAndSpans
...
sealed interface ReminderSettingsEvent {
    ...
    data class BackupImported(val count: Int) : ReminderSettingsEvent
    data object BackupUnavailable : ReminderSettingsEvent
    data object BackupImportFailed : ReminderSettingsEvent
}
...
class ReminderViewModel(
    ...
    fun importBackupJson(backupJson: String) {
        viewModelScope.launch {
            ...
            if (count != null) {
                hasBackup.value = repository.hasBackup()
                backupSavedAtMillis.value = repository.getBackupSavedAtMillis()
                _settingsEvents.tryEmit(ReminderSettingsEvent.BackupImported(count))
            } else {
                _settingsEvents.tryEmit(ReminderSettingsEvent.BackupImportFailed)
            }
        }
    }
    ...
    private data class ReminderSettingsUiState(
        val settings: com.scott.myclipboard.data.ReminderSettings,
